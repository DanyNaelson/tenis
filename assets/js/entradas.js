...
$(document).ready(function(){
	...
	$("#find_model").on("click", function(){
		...
		if(marca != "0" || modelo.trim() != ""){
			$.ajax({
			    // la URL para la petición
			    url : "obtener_producto_modelo/" + marca + "/" + modelo,
	...
	$("#send_sel").on("click", function(){
		...
		for (i = 0 ; i < tr_count ; i++) {
			...
			if($(tr_current).find(".sel_check").prop("checked")){
				...
				tr_class = $("#tabla_entradas").find("tbody").find(".producto_" + productos[0].id_producto);
				...
				if(td_current.length > 0) {
					...
					$("#tabla_entradas tbody").prepend(html_tr);
	...
	$("#finalizar").on("click", function(){
		tbody = $("#tabla_entradas").find("tbody");
		id_almacen = $("#almacen").val();
		count_tr = $(tbody).find("tr").length;

		if(id_almacen != 0){
			if(count_tr > 1){
				bootbox.confirm("Estás seguro de finalizar la entrada?", function(result) {
					if(result){
						get_values_entrie(tbody);
					}
				});
			}else{
				bootbox.alert("Necesitas agregar productos para registrar una entrada.");
			}
		}else{
			bootbox.alert("Debes seleccionar el almacén donde se hará la entrada.");
			$("#almacen").focus();
		}
	});

	/*$("#pausar").on("click", function(){
		var tbody = $("#tabla_entradas").find("tbody");
		...
		if(id_almacen != 0){
			if(count_tr > 1){
				bootbox.confirm("Estás seguro de pausar la entrada?", function(result) {
					if(result){
						var html_tbody_e = $(tbody).html();
						var html_tbody_p = $(tbody_p).html();

						var almacenes = [
											{ 
												idalmacen : id_almacen, 
											  	tbody_e : html_tbody_e, 
											  	tbody_p : html_tbody_p
											}
										];

						crear_json(almacenes);
					}
				});
			}else{
				bootbox.alert("Necesitas agregar productos para pausar una entrada.");
			}
		}else{
			bootbox.alert("Debes seleccionar el almacén donde se hará la entrada.");
			$("#almacen").focus();
	...
	$("#cancelar").on("click", function(){
		...
		if(count_tr > 1){
			bootbox.confirm("Estás seguro de cancelar la entrada?", function(result) {
				if(result){
					tbody_clean();
				}
			});
		}else{
			bootbox.alert("Necesitas agregar productos para cancelar una entrada.");
		}
	});
});
...
function remove_tr(obj_button){
	...
	if(quantity_current > 1){
		...
		bootbox.confirm("Estás seguro de borrar el producto de la entrada?", function(result) {
			if(result){
...
function remove_tr_prod(id_prod){
	count_class = $("#tabla_entradas").find(".producto_" + id_prod).length;
			  	...
			  	if(isNaN(quantity_remove)){
					...
					if(quantity_remove > 0){
						if(quantity_remove > quantity_current){
							bootbox.alert('La cantidad a borrar debe ser menor o igual a la cantidad en la entrada.');
						}else{
...
function update_quantity(operation, quantity_remove){
	quantity_current = parseInt($("#total_e").text());
...
function get_values_entrie(tbody_entries){
	entrie = new Object;
	entrie_detail = new Array();

	entrie.cantidad = parseInt($("#total_e").text());
	entrie.id_almacen = id_almacen;

	for(i = 0 ; i < (count_tr - 1) ; i++){
		tr_id = $(tbody_entries).find("tr").eq(i).attr("class");
		tr_id_producto = tr_id.split(" ");
		id_producto_tr = tr_id_producto[1].split("_");

		tr_talla = $(tbody_entries).find("tr").eq(i).find("td").eq(3).attr("class");
		tr_id_talla = tr_talla.split("_");
		id_talla_tr = tr_id_talla[1];

		cantidad = $(tbody_entries).find("tr").eq(i).find(".cantidad").text();

		entrie_detail[i] = new Object;

		entrie_detail[i].id_producto = id_producto_tr[1];
		entrie_detail[i].id_talla = id_talla_tr;
		entrie_detail[i].cantidad = cantidad;
	}

	send_values_entrie(entrie, entrie_detail);
}

function send_values_entrie(entrie, entrie_detail){
	$.ajax({
	    // la URL para la petición
	    ...
	    //datos enviados mediante post
	    data: { obj_entrie : entrie,
	    		obj_entrie_detail: entrie_detail },
	    ...
	    // código a ejecutar si la petición es satisfactoria;
	    // la respuesta es pasada como argumento a la función
	    success : function(respuesta_entrada) {
	    	respuesta_e = respuesta_entrada.split("|");
	    	bootbox.alert(respuesta_e[0]);
...
function tbody_clean(){
	tbody_e = $("#tabla_entradas").find("tbody");
	tr_current = $(tbody_e).find("tr");
...
/*function crear_json(obj_json){
	$.ajax({
		url : "crear_json",
		method : "POST",
		data : { datos_json : obj_json },
		dataType : "json",
		success : function(respuesta_archivo){
			var respuesta = respuesta_archivo.resp.split("|");
			bootbox.alert(respuesta[0]);
			if(respuesta[1] == 't'){
				//location.href = "index";
				alert('bien');
			}
